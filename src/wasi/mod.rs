...
// firebox#VPB: this is the `wasix-0.2.169` mirror — the crate
// `firebox-rust`'s `library/std/Cargo.toml` pins, so a stale table here is
// what `std` ITSELF compiles against. `firebox-patches` (0.2.174) and
// `firebox-patches-0.2.186` are the other two copies. All three must ship in
// the SAME release.
...
pub const O_TTY_INIT: c_int = 0o0;
...
// change. `src/wasi/wasi.rs` and `src/wasi/wasix.rs` define none of these, so
// `wasm32-wasmer-wasi` inherits this table verbatim.
...
pub const EMEMVIOLATION: c_int = 135;
pub const EUNKNOWN: c_int = 136;
pub const EPENDING: c_int = 137;
